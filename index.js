...
const toUnitName = amt => isNaN(Math.log10(amt)) ? "0" : (`${(amt/Math.pow(10,Math.floor(Math.log10(Math.floor(amt))/3)*3)).toFixed(3)} ${unitList[Math.floor(Math.log10(Math.floor(amt))/3)]}`);
